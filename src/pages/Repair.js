...
//   const id = searchParams.get("id"); // this is your category_id
...
//     if (brandId) {
//       fetchModels(brandId); // Always fetch models
//     }
//   };
...
//       const { response } = await apiHelper(
//         "GET",
//         `repair/models/${id}`, // No brand_id here
//         {},
//         null
//       );
//       if (response) {
...
// Same imports...
// ✅ Fully Updated Repair.jsx Component
...
const Repair = () => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const id = searchParams.get("id"); // category_id

  const [userId, setUserId] = useState(2); // Set static or retrieve from auth
  ...
  const fetchModels = async (brandId) => {
    try {
      ...
      const rawModels = response?.data?.response?.data || [];
      const formatted = rawModels.map((m) => ({ value: m.id, label: m.model }));
      setModels(formatted);
    } catch (err) {
      toast.error("Failed to fetch models");
    }
...
const onStore = async () => {
  if (!selectedBrand || !selectedModel || !selectedIssues) {
    toast.error("Please select brand, model, and issue.");
    return;
  }
        ...
        <Container>
          <Row>
            <Col lg={6} md={6} className="repair_left mb-3">
              ...
              <div className="brand-container d_flex">
                {issues.map(({ id, img, alt, issue }) => {
                  const isSelected = selectedIssues === issue; // compare by issue text now
                  return (
                    <div
                      key={id}
                      className={`brand-item issues${
                      ...
                      onClick={() => handleSelectIssue(issue)} // pass issue text
                    >
                      {img && <Image src={img} alt={alt} />}
                      <p className="issuesText">{issue}</p>
                    </div>
